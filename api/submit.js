...
module.exports = async function handler(req, res) {
  ...
  if (!AIRTABLE_TOKEN) return res.status(500).json({ error: 'Missing AIRTABLE_TOKEN' });
  if (!AIRTABLE_BASE_ID) return res.status(500).json({ error: 'Missing AIRTABLE_BASE_ID' });

  const body = req.body;
  if (!body || !body.formType) return res.status(400).json({ error: 'Missing form data' });
